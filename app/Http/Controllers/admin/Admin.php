...
class Admin extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $data['withdraw_today'] = Orders::where('status', 2)
            ->whereDate('created_at', date('Y-m-d'))
            ->count();

        // นับจำนวน Orders ที่ status = 2 และ created_at ในเดือนนี้
        $data['withdraw_month'] = Orders::where('status', 2)
            ->whereYear('created_at', date('Y'))
            ->whereMonth('created_at', date('m'))
            ->count();

        // นับจำนวน Orders ที่ status = 2 และ created_at ในปีนี้
        $data['withdraw_year'] = Orders::where('status', 2)
            ->whereYear('created_at', date('Y'))
            ->count();

        // นับจำนวน Orders ทั้งหมด ที่ status = 2
        $data['withdraw_total'] = Orders::where('status', 2)->count();
        ...
        for ($i = 1; $i <= 12; $i++) {
            $query = Orders::select(DB::raw("SUM(total) as total"))
                ...
                ->first();
            $item_mouth[] = $query->total ?? 0; // กรณี null ให้เป็น 0
        }
    ...
    {
        // ดึงข้อมูลจริง (execute query ด้วย get())
        $orders = Orders::select('orders.*', 'students.student_id as student_id', 'students.full_name as full_name')
        ...
        if ($orders->count() > 0) {
            foreach ($orders as $rs) {
                ...
                $info[] = [
                    'student_id' => $rs->student_id,       // เพิ่ม student_id
                    'full_name' => $rs->full_name,         // เพิ่ม full_name
